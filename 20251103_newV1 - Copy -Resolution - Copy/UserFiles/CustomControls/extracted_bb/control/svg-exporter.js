...
function getChartSvgMarkup() {
  ...
  if (chart.scales) {
    const visibleYScaleIds = Object.keys(chart.scales).filter(
      (id) => id !== "x" && chart.scales[id].options.display !== false,
    );
    const firstVisibleYScaleId =
      visibleYScaleIds.find((id) => chart.scales[id].position === "left") ||
      visibleYScaleIds[0];
    ...
    Object.keys(chart.scales).forEach((scaleId) => {
      ...
      if (isHorizontal) {
        ...
        body.push(
          `<line x1="${plotLeft}" y1="${yPos}" x2="${plotRight}" y2="${yPos}" stroke="#666666" stroke-width="1"/>`,
        );
          ...
          scale.ticks.forEach((tick, index) => {
            ...
            if (xPx >= plotLeft - 0.5 && xPx <= plotRight + 0.5) {
              ...
              if (xPx > plotLeft + 1 && xPx < plotRight - 1) {
                body.push(
                  `<line x1="${xPx.toFixed(2)}" y1="${plotTop}" x2="${xPx.toFixed(2)}" y2="${plotBottom}" stroke="#e5e7eb" stroke-width="1" stroke-dasharray="2,2"/>`,
                );
              }
              // Tick mark line
              body.push(
                `<line x1="${xPx.toFixed(2)}" y1="${yPos}" x2="${xPx.toFixed(2)}" y2="${yPos + 4}" stroke="#666666" stroke-width="1"/>`,
              );
              // Tick text
        ...
        if (
          ...
          body.push(
            `<text x="${titleX}" y="${titleY}" font-family="Arial, sans-serif" font-size="${titleFontSize}" font-weight="${isBold ? "bold" : "normal"}" fill="${titleColor}" text-anchor="middle">${escapeXml(titleText)}</text>`,
          );
        }
      } else {
        // Vertical Y scale
        ...
        body.push(
          `<line x1="${xPos}" y1="${plotTop}" x2="${xPos}" y2="${plotBottom}" stroke="#666666" stroke-width="1"/>`,
        );
            ...
            if (yPx >= plotTop - 0.5 && yPx <= plotBottom + 0.5) {
              ...
              if (
                ...
                if (yPx > plotTop + 1 && yPx < plotBottom - 1) {
                  ...
                  body.push(
                    `<line x1="${plotLeft}" y1="${yPx.toFixed(2)}" x2="${plotRight}" y2="${yPx.toFixed(2)}" stroke="${gridColor}" stroke-width="1"/>`,
                  );
                }
              }
              // Tick mark line
              ...
              body.push(
                `<line x1="${xPos}" y1="${yPx.toFixed(2)}" x2="${xTickEnd}" y2="${yPx.toFixed(2)}" stroke="#666666" stroke-width="1"/>`,
              );
              // Tick text
              const label = tick.label || scale.getLabelForValue(tick.value);
              if (label) {
        ...
        if (
          ...
          body.push(
            `<text x="0" y="0" font-family="Arial, sans-serif" font-size="${titleFontSize}" font-weight="${isBold ? "bold" : "normal"}" fill="${titleColor}" text-anchor="middle" transform="translate(${titleX}, ${titleY}) rotate(${rotation})">${escapeXml(titleText)}</text>`,
          );
        }
      }
    });
  ...
  if (Array.isArray(eventsList)) {
    ...
    eventsList.forEach((ev) => {
      ...
      if (Number.isFinite(xPos) && xPos >= plotLeft && xPos <= plotRight) {
        ...
        // Vertical line
        body.push(
          `<line x1="${xPos.toFixed(2)}" y1="${plotTop}" x2="${xPos.toFixed(2)}" y2="${plotBottom}" stroke="${color}" stroke-width="1"/>`,
        );
        // Rotated event text
        const textY = plotTop + 10;
        body.push(
          `<text x="${xPos.toFixed(2)}" y="${textY}" font-family="Arial, sans-serif" font-size="10" fill="${color}" transform="rotate(90, ${xPos.toFixed(2)}, ${textY})">${escapeXml(ev.text)}</text>`,
        );
      }
    });
  }
  ...
  const datasets = Array.isArray(chart.data && chart.data.datasets)
    ? chart.data.datasets
    : [];
  const legendItems = [];
  ...
  if (
    chart.legend &&
    Array.isArray(chart.legend.legendItems) &&
  ...
  if (legendItems.length > 0) {
    const legendBox =
      chart.legend &&
      Number.isFinite(chart.legend.left) &&
      Number.isFinite(chart.legend.top)
        ? {
    ...
    legendItems.forEach((item) => {
      ...
      body.push(
        `<rect x="${cursorX}" y="${rectY}" width="${rectW}" height="${rectH}" fill="${swatchColor}" stroke="none"/>`,
      );
      body.push(
        `<text x="${cursorX + rectW + 8}" y="${cursorY + 12}" font-family="Arial, sans-serif" font-size="${legendFontSize}" fill="#333333">${label}</text>`,
      );
      ...
      metaData.forEach((element) => {
        ...
        if (drawW > 0) {
          const fill =
            ...
            "#3b82f6";
          const stroke =
            element.options?.borderColor || dataset.borderColor || "none";
          const strokeWidth =
            element.options?.borderWidth || dataset.borderWidth || 0;

          body.push(
            `<rect x="${drawX.toFixed(2)}" y="${rectY.toFixed(2)}" width="${drawW.toFixed(2)}" height="${rectHeight.toFixed(2)}" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}"/>`,
          );
        }
      });
    }
  ...
  datasets.forEach((dataset, datasetIndex) => {
    ...
    dataset.data.forEach((point, pointIndex) => {
      ...
      const scaleForY =
        chart.scales && chart.scales[yAxisId] ? chart.scales[yAxisId] : null;
    ...
    if (points.length >= 2) {
      const strokeColor =
        dataset.borderColor || dataset.pointBorderColor || "#3b82f6";
      const strokeWidth = dataset.borderWidth || 2;
      body.push(
        `<polyline fill="none" stroke="${strokeColor}" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" points="${points.join(" ")}"/>`,
      );
    }
  });
...
function getExportSettings() {
  ...
  let width = parseInt(
    document.getElementById("exportWidth")?.value || "1600",
    10,
  );
  let height = parseInt(
    document.getElementById("exportHeight")?.value || "900",
    10,
  );
...
function logExportSettingsChange() {
  ...
  if (info)
    info.textContent =
      s.paper + " | " + s.orientation + " | " + s.width + "x" + s.height;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  ["paperSize", "orientation", "exportWidth", "exportHeight"].forEach(
    function (id) {
